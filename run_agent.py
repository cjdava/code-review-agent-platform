...
def main() -> None:
    ...
    frameworks = resolve_frameworks_for_repo(args.owner, args.repo, framework_hint=args.framework)
    packs = get_framework_packs(frameworks)
    primary_pack = packs[0] if packs else get_framework_pack(frameworks[0] if frameworks else "dotnet")
    standards_content = combine_standards(packs) or get_standards_content(frameworks[0], args.standards_repo_url)
    result = run_agent(
        args.owner,
        args.repo,
        args.pr,
